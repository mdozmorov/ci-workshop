...
#####################
# PAMR (Prediction Analysis of Microarrays) and SVM (Support Vector Machines)
##################### 
...
Let's install the package we'll be using:
...
With this command you train PAM on the dataset. Typing response.train you get an output showing dependence of the number of nonzero (predictive) genes from the threshold, and corresponding misclassification error.
...
4 samples belong to FRD1 class. Only 1 is correctly classified, others are classified as belonging to other classes. 4 samples in FRD440 class, on the other hand, are always correctly classified, resulting in 0 error rate. How many samples in "uninfected" class are correctly identified?
...
> data<-data[,10000] # Subset 10,000 gene, for speed purposes
> labels<-response.data$y # Class labels

Don't get confused with the dimensions of the gene expression matrix. Usually columns are samples and rows are genes. But the SVM software needs it just the other way round: the 19 rows stand for the samples and the 22283 columns for the genes.

Training and cross validation. We will begin with the simple linear kernel.
